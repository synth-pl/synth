...
const total_party_hp = (() => {
  ...
  return (party) => {
    ...
    if (__cache.has(__key)) return __cache.get(__key);
    const __result = (() => {
      return synth_sum(synth_map(synth_filter(party, __x => __x.alive), __x => __x.hp));
    })();
    __cache.set(__key, __result);
    return __result;
  };
...
const party_power_rating = (() => {
  ...
  return (party) => {
    ...
    if (__cache.has(__key)) return __cache.get(__key);
    const __result = (() => {
      return synth_sum(synth_map(party, hero_power));
    })();
    __cache.set(__key, __result);
    return __result;
  };
 ...
 * @returns {number}
 */
const count_alive = (party) => synth_count(synth_filter(party, __x => __x.alive));
